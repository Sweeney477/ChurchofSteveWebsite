          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {images.map((img, i) => (
              <div key={img._id} className={`relative overflow-hidden group rounded-lg ${i % 3 === 0 ? 'md:col-span-2 md:row-span-2 h-[600px]' : 'h-[300px]'}`}>
                <img src={img.image ? urlFor(img.image).url() : "https://via.placeholder.com/600x400"} alt={img.caption} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-1000 grayscale group-hover:grayscale-0" />
